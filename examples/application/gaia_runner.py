	...
	oxy_space.append(
		oxy.ReActAgent(
			...
			additional_prompt=(
				...
				"OUTPUT: Return ONLY the requested fact/value; no extra commentary.\n"
			),
			timeout=120,
		)
	)
	...
	oxy_space.append(
		oxy.ReActAgent(
			...
			prompt=(
				...
				"- Lists: split → trim → deduplicate → preserve order (unless sorting required) → join with ', '.\n"
...
async def run_batch(
    ...
    # 如果希望用外部传进来的文件，把下面这行换成 input_jsonl_path 即可
    jsonl_file_path = "./valid/data.jsonl"
    datasets = load_jsonl_dataset(jsonl_file_path)
    logger.info(f"Loaded dataset: {len(datasets)} examples.")

    datasets = [
        d for d in datasets
        if not (d["task_id"] in processed and processed[d["task_id"]] != "")
    ]
    datasets_df = pd.DataFrame(datasets) if datasets else pd.DataFrame()
    logger.info(f"To do: {len(datasets)} tasks.")
    ...
    with open(output_jsonl_path, "w", encoding="utf-8") as fout:
        async with MAS(oxy_space=oxy_space) as mas:
            for _, item in datasets_df.iterrows():
                answer = ""
                try:
                    question = compose_question(item)
                    if not question:
                        res = {"error": "no question field", **item}
                    ...
                    # 落盘当前结果
                    res = {**item, "answer": answer}
                    fout.write(json.dumps(res, ensure_ascii=False) + "\n")

                    save_result(
                        item, answer, result_dir,
                        checkpoint_file, failed_checkpoint_file,
                        is_error=False,
                    )
                    input()
                except Exception as e:
                    res = {**item, "answer": answer, "error": str(e)}
                    fout.write(json.dumps(res, ensure_ascii=False) + "\n")
                    save_result(
                        item, answer, result_dir,
                        checkpoint_file, failed_checkpoint_file,
    ...
    if validate and total_count:
        accuracy = correct_count / total_count
        print(f"\nValidation Accuracy: {correct_count}/{total_count} = {accuracy:.4f}")

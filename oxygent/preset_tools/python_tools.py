...
def final_answer_guard(
    ...
    answer_type: str = Field(default="number", description="Answer type: 'integer','float','number','fraction','string','list','url','text'")
) -> Dict[str, Any]:
    ...
    try:
        ...
        if action == "commit":
            ...
            if answer_type not in ["integer", "float", "number", "fraction", "string", "list", "url", "text"]:
            ...
            if answer_type in ("number", "integer"):
                if isinstance(answer, str):
                    ...
                    if answer_type in ("number", "integer"):
                        answer = int(answer) if isinstance(answer, float) and hasattr(answer, 'is_integer') and answer.is_integer() else int(answer) if isinstance(answer, float) else answer
                else:
                ...
                try:
                    if isinstance(answer, str):
                        ...
                        return {"ok": False, "error": f"Invalid float type: {type(answer)}"}
                except Exception as e:
                    return {"ok": False, "error": f"float_parse_error: {e}"}

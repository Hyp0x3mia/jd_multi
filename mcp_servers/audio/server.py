...
async def audio_transcribe(
    ...
    prompt: Optional[str] = Field(default="请将音频转写为文本", description="可选：转写提示词，用于限定领域或人名"),
    ...
    try:
        ...
        return {
            "status": "success",
            "analysis": "ASR未检测到任何文本,可能文件是纯音乐，请使用audio_recognize_song工具识别音乐名后交叉确认TimeOut"
        }
    except Exception as e:
        error_str = str(e)
        # 区分不同类型的错误
        return {
            "status": "success",
            "analysis": "ASR未检测到任何文本,可能文件是纯音乐，请使用audio_recognize_song工具识别音乐名后交叉确认Model"
        }
...
if __name__ == "__main__":
    ...
    #         video_path = "/Users/hyp0x3mia/BUPT_Master/Master/2025/开源之夏/京东/OxyGent 2/test/大满贯.mp3"

    #         # 示例 2：简单问题回答
    #         prompt2 = "用户加入购物车的第一个商品内存版本比相同配色的最小内存版本大了多少？单位GB，直接输出数字"
    #         answer = await  audio_transcribe(video_path )
